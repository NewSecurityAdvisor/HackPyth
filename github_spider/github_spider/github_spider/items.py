...
class GitspiderSpider(scrapy.Spider):
    ...
    def check_for_sensitive_data(self, repo_directory):
        ...
        # Run truffleHog on the cloned repository and capture output
        trufflehog_cmd = f'trufflehog filesystem --directory={repo_directory}'
        try:
            output = subprocess.check_output(trufflehog_cmd, shell=True, stderr=subprocess.STDOUT)
            output = output.decode('utf-8').strip()

            # Check if truffleHog found any issues
            if output:
                with open(findings_file, 'a') as findings:
                    findings.write(f'truffleHog findings in {repo_directory}:\n{output}\n\n')

        except subprocess.CalledProcessError as e:
            self.log(f'truffleHog error in {repo_directory}: {e.output.decode("utf-8").strip()}')